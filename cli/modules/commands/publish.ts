...
export const publish = async () => {
  ...
  if (packageDomain) {
    ...
    const existingMetadata: any = await response.json()

    const versionAlreadyPublished = Object.keys(
  ...
  if (!packageDomain) {
    ...
    const previousMetadata = (await response.json()) as Packument
  ...
  const metadataTags: { name: string; value: string }[] = [
  ...
  fs.writeFileSync(
    `${OMNITORY_DIRECTORY}/${packageJson.name}-${packageJson.version}-${metadataUploadResult.id}.json`,
    metadataString
  )
