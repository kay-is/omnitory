...
export const getTarballShasum = async (tarballPath) => {
...
export async function signMessage(message) {
    ...
    // @ts-expect-error
    const { subtle } = crypto;
    const privateKey = await subtle.importKey("jwk", await getKey(), { name: "RSA-PSS", hash: "SHA-256" }, false, ["sign"]);
    const encodedMessage = new util.TextEncoder().encode(message);
    const hash = await subtle.digest("SHA-256", encodedMessage);
    const signature = await subtle.sign({ name: "RSA-PSS", saltLength: 32 }, privateKey, hash);
